...
\begincols
  ...
  **What you need** (*Dependencies*) \newline
  ...
  \pause
  *Advanced* use of `Rmarkdown` is usually fuelled through \LaTeX \hspace{.04cm} and so you'll need installations of:  
  \begin{itemize}
...
\begin{itemize}
\pause
\item Figures are generated on each compilation (unless cache is stored) thus always being up-to-date with the underlying data
\vspace{.1cm}
\item Sub-figures are established automatically as long as sub-captions are supplied
\vspace{.1cm}
...
The `kable` functionality in `Rmarkdown` enables us to generate appealing \LaTeX\hspace{.04cm}-styled tables directly via `R` coding: 
...
\vspace{.1cm}
This can be circumvented by defining novel commands for the column environment instead of calling the default \LaTeX\hspace{.04cm} commands.
