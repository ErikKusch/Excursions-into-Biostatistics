...
\begincol{.5\linewidth}
  ...
  \tiny
  \begin{equation}
  \ell(\beta_{0},\beta_{1}) = \prod\limits_{i:y_{i}=1} Pr(x_{i}) \prod\limits_{i':y_{i'}=1} (1-Pr(x_{i'}))
  \end{equation}
\endcol
\endcols
...
- \textbf{Generalized Linear Model} `glm`. When the assumptions of \textbf{homoscedasticity} and \textbf{homoscedasticity} are \textbf{violated}.  

\vspace{.2cm}

- \textbf{Generalized Linear Mixed Effect Model} `glmmPQL` from `MASS`, or `glmer` from `lme4`. When the assumptions of \textbf{homoscedasticity}, \textbf{homoscedasticity}, and \textbf{independence} are \textbf{violated}.  
